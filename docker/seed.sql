...
INSERT INTO dashboard_alerts (id, usuario, departamento, "centroTrabajo", rol, estado, "statusType", "sortOrder", "isActive", "createdAt", "updatedAt") VALUES
    (gen_random_uuid(), 'Isabella Torres', 'Comercial', 'Barakaldo',  'Tienda',        'No ha fichado',            'no-fichado',    1, true, NOW(), NOW()),
    (gen_random_uuid(), 'Maria Robledo',   'Comercial', 'Las Arenas', 'Telemarketing', 'No ha fichado',            'no-fichado',    2, true, NOW(), NOW()),
    (gen_random_uuid(), 'Lucia Martinez',  'Comercial', 'Las Arenas', 'Comercial',     'No ha fichado',            'no-fichado',    3, true, NOW(), NOW()),
    (gen_random_uuid(), 'Isabella Torres', 'Comercial', 'Barakaldo',  'Comercial',     'No ha fichado',            'no-fichado',    4, true, NOW(), NOW()),
    (gen_random_uuid(), 'Maria Robledo',   'Comercial', 'Las Arenas', 'Tienda',        'No ha fichado',            'no-fichado',    5, true, NOW(), NOW()),
    (gen_random_uuid(), 'Lucia Martinez',  'Comercial', 'Las Arenas', 'Telemarketing', 'Fichaje fuera de horario', 'fuera-horario', 6, true, NOW(), NOW()),
    (gen_random_uuid(), 'Lucia Martinez',  'Comercial', 'Las Arenas', 'Comercial',     'Fichaje fuera de horario', 'fuera-horario', 7, true, NOW(), NOW());
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'clients') THEN
    ...
    SELECT 'c1000001-0000-0000-0000-000000000001', 'Cliente A', 'cliente.a@example.com', '+34 600 111 222', NOW(), NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients LIMIT 1)
    UNION ALL
    SELECT 'c1000001-0000-0000-0000-000000000002', 'Cliente B', 'cliente.b@example.com', '+34 600 333 444', NOW(), NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients LIMIT 1)
    UNION ALL
    SELECT 'c1000001-0000-0000-0000-000000000003', 'Cliente C', 'cliente.c@example.com', NULL, NOW(), NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients LIMIT 1);
  END IF;
END $$;

#!/usr/bin/env node
/**
 * Smoke test: prueba todos los endpoints de la API (alineados con Swagger).
 * Requiere: API levantada (pnpm start o pnpm dev:api).
 * Uso: pnpm api:smoke   o   node scripts/api-smoke-test.js
 * Env: API_URL (default http://localhost:4000), API_SMOKE_USER, API_SMOKE_PASSWORD (opcional).
 */
...
const ids = {}; // ids creados o obtenidos para reutilizar
...
async function run() {
  console.log(`\nAPI Smoke Test (todos los endpoints) -> ${API}\n`);
  ...
  if (!loginRes.ok) {
    ...
    console.log('Result: OK=', results.ok, 'FAIL=', results.fail, 'SKIP=', results.skipped);
    process.exit(1);
    return;
  }
  ...
  if (token) pass('POST /auth/login'); else fail('POST /auth/login', loginRes.status, 'no access_token');

  // --- Health ---
  for (const path of ['/health', '/health/live', '/health/ready']) {
    const r = await request('GET', `${API}${path}`, token);
    if (r.status === 404) {
      const r2 = await request('GET', `${HEALTH}${path}`, token);
      if (r2.ok) pass(`GET ${path}`); else fail(`GET ${path}`, r2.status, r2.body);
    } else if (r.ok) pass(`GET ${path}`); else fail(`GET ${path}`, r.status, r.body);
  }
  ...
  if (meRes.ok) {
    ...
    ids.userId = me?.id || me?.userId;
  } else fail('GET /auth/me', meRes.status, meRes.body);

  const forgotRes = await request('POST', `${API}/auth/forgot-password`, null, { email: USER });
  if (forgotRes.ok) pass('POST /auth/forgot-password'); else fail('POST /auth/forgot-password', forgotRes.status, forgotRes.body);
  ...
  if (usersRes.ok) {
    ...
    if (!ids.userId) {
      ...
      if (list.length) ids.userId = list[0].id;
    }
  } else fail('GET /users', usersRes.status, usersRes.body);

  if (ids.userId) {
    const r = await request('GET', `${API}/users/${ids.userId}`, token);
    if (r.ok) pass('GET /users/:id'); else fail('GET /users/:id', r.status, r.body);
  } else skip('GET /users/:id', 'no userId');

  const createUserRes = await request('POST', `${API}/users`, token, {
  ...
  if (ids.createdUserId) {
    ...
    if (r.ok) pass('PUT /users/:id'); else fail('PUT /users/:id', r.status, r.body);
  }

  // --- Clients ---
  const clientsRes = await request('GET', `${API}/clients`, token);
  if (clientsRes.ok) pass('GET /clients'); else fail('GET /clients', clientsRes.status, clientsRes.body);

  const createClientRes = await request('POST', `${API}/clients`, token, {
    name: 'Smoke Client',
    email: `smoke-${Date.now()}@client.test`,
    phone: '+34600000000',
  });
  if (createClientRes.ok) pass('POST /clients'); else fail('POST /clients', createClientRes.status, createClientRes.body);

  // --- Dashboard ---
  const homeRes = await request('GET', `${API}/dashboard/home`, token);
  if (homeRes.ok) pass('GET /dashboard/home'); else fail('GET /dashboard/home', homeRes.status, homeRes.body);
  ...
  if (ids.terminalObjectiveId) {
    ...
    if (r.ok) pass('PATCH /dashboard/terminal-objectives/:id'); else fail('PATCH /dashboard/terminal-objectives/:id', r.status, r.body);
  } else skip('PATCH /dashboard/terminal-objectives/:id', 'no objective id');

  // --- Empresa ---
  const empresaRes = await request('GET', `${API}/empresa`, token);
  if (empresaRes.ok) pass('GET /empresa'); else fail('GET /empresa', empresaRes.status, empresaRes.body);
  ...
  if (ids.departmentId) {
    ...
    if (r.ok) pass('GET /empresa/departments/:id'); else fail('GET /empresa/departments/:id', r.status, r.body);
  }
  if (ids.createdDepartmentId) {
    ...
    if (r.ok) pass('PUT /empresa/departments/:id'); else fail('PUT /empresa/departments/:id', r.status, r.body);
  }
  ...
  // Same body shape as Swagger default (departmentId invalid string must not cause 500)
  const createWcRes = await request('POST', `${API}/empresa/work-centers`, token, {
  ...
  if (ids.workCenterId) {
    ...
    if (r.ok) pass('GET /empresa/work-centers/:id'); else fail('GET /empresa/work-centers/:id', r.status, r.body);
  }
  if (ids.createdWorkCenterId) {
    ...
    if (r.ok) pass('PUT /empresa/work-centers/:id'); else fail('PUT /empresa/work-centers/:id', r.status, r.body);
  }

  // --- Fichajes ---
  const fichajesRes = await request('GET', `${API}/fichajes`, token);
  if (fichajesRes.ok) pass('GET /fichajes'); else fail('GET /fichajes', fichajesRes.status, fichajesRes.body);

  if (ids.userId) {
    const r = await request('GET', `${API}/fichajes/user/${ids.userId}`, token);
    if (r.ok) pass('GET /fichajes/user/:userId'); else fail('GET /fichajes/user/:userId', r.status, r.body);
  ...
  if (ids.userId) {
    ...
    if (r.ok) pass('GET /fichajes/current'); else fail('GET /fichajes/current', r.status, r.body);
  ...
  if (ids.fichajeId) {
    ...
    if (pauseRes.ok) pass('POST /fichajes/:fichajeId/pause'); else fail('POST /fichajes/:fichajeId/pause', pauseRes.status, pauseRes.body);
    const resumeRes = await request('POST', `${API}/fichajes/${ids.fichajeId}/resume`, token);
    if (resumeRes.ok) pass('POST /fichajes/:fichajeId/resume'); else fail('POST /fichajes/:fichajeId/resume', resumeRes.status, resumeRes.body);
    const r = await request('POST', `${API}/fichajes/${ids.fichajeId}/clock-out`, token);
    if (r.ok) pass('POST /fichajes/:fichajeId/clock-out'); else fail('POST /fichajes/:fichajeId/clock-out', r.status, r.body);
  } else {
    skip('POST /fichajes/:fichajeId/pause', 'no fichaje id');
    skip('POST /fichajes/:fichajeId/resume', 'no fichaje id');
    skip('POST /fichajes/:fichajeId/clock-out', 'no fichaje id (clock-in may have failed or already closed)');
  }

  // --- Fichajes sub: calendars, schedules, permission-types ---
  const calRes = await request('GET', `${API}/fichajes/calendars`, token);
  if (calRes.ok) pass('GET /fichajes/calendars'); else fail('GET /fichajes/calendars', calRes.status, calRes.body);
  const createCalRes = await request('POST', `${API}/fichajes/calendars`, token, { name: 'Smoke Calendar', description: 'Smoke' });
  if (createCalRes.ok) pass('POST /fichajes/calendars'); else fail('POST /fichajes/calendars', createCalRes.status, createCalRes.body);

  const schedRes = await request('GET', `${API}/fichajes/schedules`, token);
  if (schedRes.ok) pass('GET /fichajes/schedules'); else fail('GET /fichajes/schedules', schedRes.status, schedRes.body);
  const createSchedRes = await request('POST', `${API}/fichajes/schedules`, token, { name: 'Smoke Schedule', hoursPerWeek: 40 });
  if (createSchedRes.ok) pass('POST /fichajes/schedules'); else fail('POST /fichajes/schedules', createSchedRes.status, createSchedRes.body);

  const permRes = await request('GET', `${API}/fichajes/permission-types`, token);
  if (permRes.ok) pass('GET /fichajes/permission-types'); else fail('GET /fichajes/permission-types', permRes.status, permRes.body);
  const createPermRes = await request('POST', `${API}/fichajes/permission-types`, token, { name: 'Smoke Permiso', isPaid: true });
  if (createPermRes.ok) pass('POST /fichajes/permission-types'); else fail('POST /fichajes/permission-types', createPermRes.status, createPermRes.body);
  ...
  if (ids.taskId) {
    ...
    if (r.ok) pass('GET /tasks/:taskId'); else fail('GET /tasks/:taskId', r.status, r.body);
  ...
  if (ids.createdTaskId) {
    ...
    if (r.ok) pass('PATCH /tasks/:taskId'); else fail('PATCH /tasks/:taskId', r.status, r.body);
    const del = await request('DELETE', `${API}/tasks/${ids.createdTaskId}`, token);
    if (del.ok) pass('DELETE /tasks/:taskId'); else fail('DELETE /tasks/:taskId', del.status, del.body);
  }
  ...
  if (ids.productId) {
    ...
    if (r.ok) pass('GET /productos/:id'); else fail('GET /productos/:id', r.status, r.body);
  ...
  if (ids.createdProductId) {
    ...
    if (r.ok) pass('PATCH /productos/:id'); else fail('PATCH /productos/:id', r.status, r.body);
  }
  ...
  if (ids.inventoryId) {
    ...
    if (r.ok) pass('GET /inventory/:id'); else fail('GET /inventory/:id', r.status, r.body);
  ...
  if (ids.createdInventoryId) {
    ...
    if (r.ok) pass('PATCH /inventory/:id'); else fail('PATCH /inventory/:id', r.status, r.body);
  }

  // --- Reports ---
  const reportRes = await request('GET', `${API}/reports/summary`, token);
  if (reportRes.ok) pass('GET /reports/summary'); else fail('GET /reports/summary', reportRes.status, reportRes.body);

  // --- Alertas ---
  const alertasRes = await request('GET', `${API}/alertas`, token);
  if (alertasRes.ok) pass('GET /alertas'); else fail('GET /alertas', alertasRes.status, alertasRes.body);

  // --- Operaciones ---
  const opRes = await request('GET', `${API}/operaciones`, token);
  if (opRes.ok) pass('GET /operaciones'); else fail('GET /operaciones', opRes.status, opRes.body);

  // --- User documents ---
  if (ids.userId) {
    const docsRes = await request('GET', `${API}/users/${ids.userId}/documents`, token);
    if (docsRes.ok) pass('GET /users/:userId/documents'); else fail('GET /users/:userId/documents', docsRes.status, docsRes.body);
    ...
    if (ids.createdDocId) {
      ...
      if (r.ok) pass('GET /users/:userId/documents/:docId'); else fail('GET /users/:userId/documents/:docId', r.status, r.body);
      const del = await request('DELETE', `${API}/users/${ids.userId}/documents/${ids.createdDocId}`, token);
      if (del.ok) pass('DELETE /users/:userId/documents/:docId'); else fail('DELETE /users/:userId/documents/:docId', del.status, del.body);
    }
  } else {

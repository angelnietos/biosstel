/**
 * @biosstel/objetivos - Objetivos Producto
 * Misma estructura que Niveles pero solo objetivos de tipo producto y título "Objetivos Producto".
 */
...
export const ObjetivosProductoPage = () => {
  ...
  const objectives = useMemo(() => allObjectives.filter((o) => getObjectiveTypeFromTitle(o.title) === 'product'), [allObjectives]);
  const hasObjectives = objectives.length > 0;

  return (
    <Stack gap={6}>
      <Heading level={1} className="text-gray-900 font-bold">Objetivos Producto</Heading>
      <FilterBar
        values={filterValues}
      ...
      {isLoading ? (
        ...
        <Card className="px-6 py-5 border border-border-card rounded-xl shadow-sm">
          <Text variant="muted">No hay objetivos de producto con los filtros aplicados. Pruebe otros filtros o vea todos en Niveles de objetivos.</Text>
        </Card>
      )}
    </Stack>
  );
...
export default ObjetivosProductoPage;

/**
 * @biosstel/alertas - Recordatorios
 * Avisos "Recuerda fichar". Layout Figma. Datos desde API (tipo=recordatorios).
 */
...
export const Recordatorios = () => {
  ...
  useEffect(() => {
    (dispatch as (thunk: ReturnType<typeof fetchAlertas>) => void)(fetchAlertas({ tipo: 'recordatorios' }));
  }, [dispatch]);

  const loadData = () => (dispatch as (thunk: ReturnType<typeof fetchAlertas>) => void)(fetchAlertas({ tipo: 'recordatorios' }));
  const isEmpty = !loading && !error && alertas.length === 0;

  return (
    <AlertasPageLayout title="Recordatorios">
      <Stack gap={6}>
            ...
            <tbody>
              ...
              {!loading && !error && alertas.length > 0 && alertas.map((a) => (
                <tr key={a.id} className="border-b border-border-card">
                  ...
                  <td className="px-4 py-3 text-muted">{a.estado ?? 'Recuerda fichar'}</td>
              ...
              {isEmpty && (
                <tr>
                  <td colSpan={3} className="px-4 py-12 text-center">
                    <Text variant="muted" className="block mb-4">No hay recordatorios.</Text>

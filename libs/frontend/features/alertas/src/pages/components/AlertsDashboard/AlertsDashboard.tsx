...
export const AlertsDashboard = () => {
  ...
  const { alerts, isLoading: alertsLoading, error: alertsError } = useSelector((state: { alertas: AlertasState }) => state.alertas);
  ...
  }, [dispatch, tipo, departamento, centroTrabajo]);

  const handleApplyFilters = () => {
    logUserAction('alertas_apply_filters', undefined, { tipo, departamento, centroTrabajo });
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-8 space-y-8">
      ...
      {/* Filters: tipo, departamento, centro (inputs + Aplicar) */}
        ...
        <div className="space-y-6">
          ...
          {alertsError && (
            <div className="rounded-lg bg-red-50 p-3 text-sm text-red-700 border border-red-100">
              {alertsError}
              <button type="button" onClick={loadAlertas} className="ml-2 font-medium underline">Reintentar</button>
            </div>
          )}

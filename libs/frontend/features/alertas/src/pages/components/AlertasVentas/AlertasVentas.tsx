/**
 * @biosstel/alertas - AlertasVentas
 * Notificaciones de objetivos/desviaciones. Layout Figma. Datos desde API.
 */
...
export const AlertasVentas = () => {
  const dispatch = useDispatch();
  const { alerts: alertas, isLoading: loading, error } = useSelector((state: { alertas: AlertasState }) => state.alertas);

  useEffect(() => {
    (dispatch as (thunk: ReturnType<typeof fetchAlertas>) => void)(fetchAlertas({ tipo: 'ventas' }));
  }, [dispatch]);

  const loadData = () => (dispatch as (thunk: ReturnType<typeof fetchAlertas>) => void)(fetchAlertas({ tipo: 'ventas' }));
  const isEmpty = !loading && !error && alertas.length === 0;

  return (
    <AlertasPageLayout title="Alertas de ventas">
      <Stack gap={6}>
            ...
            <tbody>
              ...
              {!loading && !error && alertas.length > 0 && alertas.map((a) => (
                <tr key={a.id} className="border-b border-border-card">
                  ...
                  <td className="px-4 py-3 text-muted">{a.departamento ?? '—'}</td>
              ...
              {isEmpty && (
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center">
                    <Text variant="muted" className="block mb-4">No hay alertas de ventas.</Text>

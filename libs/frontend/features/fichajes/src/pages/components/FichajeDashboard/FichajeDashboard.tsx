...
import { Link, useRouter, canFichar, canManageFichajes } from '@biosstel/platform';
import {
  clockIn,
  clockOut,
  pauseWork,
...
export const FichajeDashboard = () => {
  ...
  const showReloj = canFichar(userRole);
  const showGestionFichajes = canManageFichajes(userRole);
  ...
  useEffect(() => {
    ...
    if (canFichar(userRole) && !canManageFichajes(userRole)) {
      router.replace('/fichajes/control-jornada');
    }
  }, [authRestored, userRole, router]);
  ...
  const isEmployeeOnly = authRestored && userRole != null && canFichar(userRole) && !canManageFichajes(userRole);
  if (isEmployeeOnly) {
    return (
      <PageContainer maxWidth="xl">

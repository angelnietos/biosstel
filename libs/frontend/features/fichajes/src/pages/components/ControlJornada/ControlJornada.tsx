...
import { useRouter, useLocale, canFichar, logUserAction } from '@biosstel/platform';
...
export const ControlJornada = () => {
  ...
  useEffect(() => {
    ...
    if (!canFichar(userRole)) {
      window.location.replace(`/${locale}/fichajes`);
    }
  }, [mounted, authRestored, authUser, userRole, locale]);
  ...
  const userCanFichar = canFichar(userRole);
  if (!authUser || !userCanFichar) {
    return null;
  }

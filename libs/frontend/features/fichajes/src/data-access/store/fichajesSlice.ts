...
export const clockIn = createAsyncThunk(
    'fichajes/clockIn',
    async (data: { userId: string; location?: { lat: number; lng: number } }) => {
        return await fichajesService.clockIn(data);
    }
);
...
const fichajesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(clockIn.rejected, (state, action) => {
        state.status = 'failed';
        state.error = action.error.message || 'Error al fichar entrada';
    });

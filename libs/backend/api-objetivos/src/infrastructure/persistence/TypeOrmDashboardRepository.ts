...
import { DashboardAlertEntity } from './DashboardAlertEntity';
import { DashboardObjectiveEntity } from './DashboardObjectiveEntity';
...
export class TypeOrmDashboardRepository implements IDashboardRepository {
  ...
  async getTerminalObjectives(
    ...
    const toArray = (v: string | string[] | undefined): string[] =>
      v == null ? [] : Array.isArray(v) ? v : [v];
    const typeValues = toArray(filters?.type ?? (filters as any)?.type);
    const periodValues = toArray(filters?.period ?? (filters as any)?.period);
    ...
    if (!terminalObjective) {
      ...
      // Último objetivo inactivo para este tipo; si no hay, el último de cualquier estado para que el front tenga id al activar
